...
Een lichte voorraadtracker voor mobiele airco's in Nederland en Frankrijk, geschikt voor lokaal gebruik en een wachtwoordloze implementatie in Azure. De tracker controleert momenteel deze Nederlandse sites:
...
De Boulanger- en Brico Dépôt France-adaptercode blijft in de repository staan, maar is nog niet in productie geregistreerd: Boulanger is lokaal bereikbaar, terwijl Azure Container Apps-uitgaand verkeer consequent een read-timeout van 60 seconden krijgt; de gewone categoriepagina en het smartcache-fragment van Brico Dépôt zijn lokaal leesbaar, maar Azure krijgt te kleine/onbruikbare antwoorden. Schakel ze pas in zodra er een stabiele pagina-API of officiële/openbare alternatieve bron is gevonden.
...
Ook Franse adapters onderscheiden directe voorraad van voorverkoop: `Pré-commande`, `Expédition à partir`, `livraison prévue semaine`, `Sur commande` en levertijden van meerdere weken worden als voorverkoop getoond en veroorzaken geen e-mail voor directe voorraad. Electro Dépôt France leest de ingebedde Vue JSON-waarde `stock`; Costway France leest de Magento-voorraadklasse `qty-N` en `Précommande`-labels, maar sluit split-systemen, luchtkoelers en accessoires uit; Maison Energy laat `Non disponible`/`Demande de devis` voorgaan op schema `PreOrder`, zodat offerteproducten geen voorraadmelding veroorzaken. H2R Équipements leest alleen de camper/van-categorie `climatisation nomade`, waarbij alleen `En stock` directe voorraad is en `Sur commande`/`Retour en stock prévu` nooit als directe voorraad meldt; Obelink France leest openbare JSON-LD-categorieën en productpagina's voor mobiele/split-campingairco's; Narbonne Accessoires gebruikt het zichtbare blok `Livraison à Domicile` in plaats van schema-voorraad, zodat alleen-afhalen niet als bezorgvoorraad telt; Mon Camping Car toont `BackOrder`/`Disponible à partir` als voorverkoop. Action France geeft voor deze zoekopdracht momenteel vooral luchtkoelers/ventilatoren terug en wordt daarom streng gefilterd. Boulanger, Brico Dépôt France, Cdiscount, E.Leclerc en de Franse sites met directe 403 zijn nog niet ingeschakeld, zodat time-outs, anti-botpagina's of JS-shells niet als voorraadbron worden gebruikt.
