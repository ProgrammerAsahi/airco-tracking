...
- 已部署 backend image/commit：`1dd3017607ea0f2e6c72a223cd5aa0b6b5f5d24e`
- 已部署的兼容 frontend commit：`d097c75c9850946be024920677eba6761174ac48`
- 最新成功的 backend CI/deploy workflows：`29963989458` / `29963989419`；生产 frontend revision：`airco-tracking-web--0000066`
...
ACS higher-quota request 当前为 **Open**；其私有 case identifier 保存在这个公开仓库之外。Portal 申请档位是 `250`（1,000 封/分钟、3,000 封/小时），但应用初期会自行限制为最多 100 封/分钟、10,000 封/天，面向初期最多 1,000 用户。在 Azure 批准前继续保持已部署的一 worker/13 秒限制；批准后用两到四周逐步 warm up。
...
- Web、scanner、retention、publisher、fan-out、email delivery 和 delivery-report processing 使用相互分离的身份。生产中 `aircontrack-identity` 绑定 Web app，`aircontrack-scanner` 绑定 scanner job，`aircontrack-retention` 绑定后端 retention，`aircontrack-web-retention` 绑定 Web 认证 cleanup，`aircontrack-alert-publisher` 绑定 publisher，`aircontrack-alert-fanout` 绑定 reconciler/coordinator/fan-out workers，`aircontrack-alert-email` 绑定 email worker，`aircontrack-alert-delivery-report` 绑定 delivery processing 与其 DLQ cleanup。流水线权限在 Azure RBAC 支持的范围内限制到具体 entity/table。Vault-wide secret access 已换成精确 secret scope：Web 仅能读取 unsubscribe/withdrawal/OTP pepper，scanner 仅能读取 Awin/AliExpress 凭据，email 仅能读取 unsubscribe key。只有 Event Grid 因 Azure managed-identity dead-letter validation 要求保留 storage-account scope Blob role；delivery publisher 仅拥有 `alertdeliveries` 表级读取权限。GitHub 通过 OIDC 和 least-privilege custom role 部署，不能创建 role assignments 或读取应用 secrets。`infra/github-oidc.bicep` 会在 `airco-github-deployer` 上同时创建 ref-scoped 和 environment-scoped 的 federated credentials（`github-airco-tracking-env-production`、`github-airco-tracking-web-env-production`），因为声明 `environment:` 的 job 会拿到 environment-scoped 的 token subject。
...
- AliExpress Affiliate Open Platform 的搜索和 SKU-detail clients 已作为按国家区分的诊断能力实现。每个请求都会携带真实配送国家，但已批准的 SKU contract 没有提供文档化的库存/可下单字段，因此这些 adapters 被刻意排除在生产 registry 之外，不能生成库存或提醒。只使用 catalog/affiliate scopes；buyer、order、payment 和其它个人数据均不在范围内。

## 已部署的加固版本

已部署版本拆分了 Web、scanner 和 retention identities；把 `alertoutboxpending` 改为保存完整 payload 的权威 enqueue journal，并提供覆盖全部 continuation pages 的旧数据迁移和崩溃安全 archive 修复；从所有实时汇总排除 stale retailer 商品，并在 24 小时诊断窗口后清空其商品 payload；让所有零售商及生产 partner API 流量统一经过有界且校验 redirect 的 fetch 边界，并为各 endpoint 配置 MIME/大小限制和显式只读 POST 重试；校验 canonical 与 affiliate URL hosts；90 天后压缩缺货商品状态、365 天后删除 tombstone；并移除 retention 固定 5,000-row 上限。CI 使用 hash-locked dependency set 验证 push `main`。

Owner-only 的 `scripts/migrate-runtime-identities.sh --apply` 已在 workload 与 smoke 验证后完成。之后再次 dry-run 得到干净 no-op：所有精确替代 grant 均存在，也没有任何枚举的 legacy grant 需要删除。GitHub 仍被刻意限制为不能执行 RBAC cleanup。完整契约、回滚、幂等和生产证据见 [HARDENING.zh.md](./HARDENING.zh.md)。

## 本次 release 已完成验证

Backend image/commit `1dd3017607ea0f2e6c72a223cd5aa0b6b5f5d24e` 已部署并完成生产验证。CI workflow `29963989458` 通过，deploy workflow `29963989419` 经必需的 `production` 审批后成功完成。

- Backend：发布前 413/413 unit tests、`compileall`、shell syntax、全部 Bicep builds、locked-dependency audit 和 `git diff --check` 均通过。
- 第一次生产 retention execution 暴露了本地 mock 未覆盖的真实 Azure SDK 契约：`TableClient.query_entities` 需要位置参数 `query_filter`。Commit `1dd3017` 已显式传入空 filter，并用回归测试断言该调用；immutable image 重新部署后，真实生产 retention rerun 成功。
- Frontend commit `d097c75c9850946be024920677eba6761174ac48` 对应生产 revision `airco-tracking-web--0000066`；`/health`、`/ready`、本地化法律内容和匿名库存拒绝均通过 smoke checks。
- Reconciler、scanner、publisher、retention、delivery cleanup 和全部四个 backend apps 均使用各自精确 identity。三条 Service Bus queues 和 stock-event subscription 最终均为 0 active / 0 dead-letter。
- Least-privilege migration 只在替代 grant 验证通过后删除枚举的宽泛 table、Blob 和 vault-wide secret grants；部署后的 dry-run 为幂等 no-op，既没有旧 grant，也没有进一步修改。
- 真实生产 ACS 验证邮件已投递到彼此独立的 Outlook 和 Gmail 收件端，custom domain 的 SPF、DKIM、DMARC 均通过，并带有 support `Reply-To`。验证时不存在 active entitled alert recipients，因此完整 fan-out canary 正确地没有生成 delivery job，而不是绕过 entitlement 检查。
...
3. 至少有一个明确同意且权益有效的测试收件人后，再执行完整 stock-event → fan-out → ACS → final-delivery canary；不要仅为强行测试而创建权益。
4. 补齐 legal 页面中的私有运营主体/VAT/退款/管辖法律事实并完成法律审查。在商业与法律上线条件全部满足前，Stripe 必须保持 test mode 并 fail closed。
