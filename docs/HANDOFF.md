...
The latest retailer rounds added stable French coverage in three batches. The active French adapters are Castorama, Auchan, Rue du Commerce, Electro Dépôt France, Costway France, Maison Energy, Create France, Evolarshop France, Klarstein France, Trotec France, De'Longhi France, Lidl France, Action France, H2R Équipements, Obelink France, Narbonne Accessoires, and Mon Camping Car. France adapters split immediate stock from presale (`Pré-commande`, `Expédition à partir`, `livraison prévue semaine`, `Sur commande`, and multi-week lead times) and aggressively filter coolers/fans/accessories/fixed split systems. The third batch intentionally added only camping/van sources with stable public data: H2R's server-rendered `climatisation nomade` category, Obelink France's JSON-LD category/product pages, Narbonne's known PDPs with visible home-delivery stock, and Mon Camping Car's portable category plus product JSON-LD. Boulanger and Brico Dépôt France adapter code is present but deferred: Boulanger is reachable locally while Azure Container Apps outbound requests consistently hit a 60-second read timeout; Brico Dépôt's normal category page and smartcache fragment are readable locally, but Azure receives too-small/unusable responses. Re-enable only after a stable page API or official/public alternative source is found. First-, second-, and third-batch French backlog sites remain deferred when direct responses are anti-bot/403, SPA shells without stable product data, brand/catalog pages without direct stock, or fixed/window/split-heavy results. Prior rounds added Bostools as Dutch retailer 28, presale separation, the inventory snapshot, 27 retailers, Azure Key Vault recipient storage, EUR 1,500 / 7,000 BTU alert filters, bol.com removal, and a BTU accuracy audit. Conrad remains pending because its public pages reject automated requests and its official API requires separate approval.
...
The application currently registers 45 credential-free adapters: 28 Dutch adapters and 17 French adapters.
...
- Third-batch backlog: Euro Accessoires and Manutan are anti-bot/captcha blocked; Alternate France currently surfaces only EcoFlow WAVE accessories rather than air conditioners; Maxiburo/Bruneau are Nuxt/SPA B2B pages without stable server-rendered product data for this query; Seton search returns no relevant air-conditioner products; Airton is mostly fixed split/monobloc installation hardware; Espace Aubade search returned 404/500-style unstable pages.
- Dedicated retailer 403/anti-bot backlog: see `docs/RETAILER_403_BACKLOG.md`.
...
- France: Auchan/Rue du Commerce parse server-rendered cards or microdata; Castorama is tracked but current products are store-availability-only and not immediate online stock; Electro Dépôt France reads embedded Vue JSON and requires `stock > 0`; Costway France reads Magento `qty-N` classes, marks `Précommande` separately, and excludes split systems/coolers/accessories; Maison Energy uses schema availability but lets `Non disponible`/`Demande de devis` override `PreOrder`; Create France and Evolarshop France expose preorders as presale; Klarstein France and De'Longhi France are currently out of stock; Trotec France uses the public Algolia product index but requires name-level filtering because the search index also returns accessories; Lidl France uses the public product sitemap; Action France currently returns coolers/fans and is allowed to succeed with zero real air conditioners; H2R Équipements uses the server-rendered `climatisation nomade` camper category and marks `Sur commande` as presale; Obelink France reads JSON-LD ItemList/product offers for mobile/split camping ACs; Narbonne Accessoires ignores schema stock unless the visible home-delivery block is `En stock`; Mon Camping Car uses the portable category and treats schema `BackOrder`/future availability as presale. Boulanger is deferred due Azure read timeout despite local accessibility. Brico Dépôt France is deferred because both normal and smartcache paths are unstable from Azure.
...
- **2026-07-06**: Added the second stable France batch (3 active adapters): Electro Dépôt France, Costway France, and Maison Energy. Brico Dépôt France parser code/tests are retained but deferred after Azure production verification returned too-small/unusable responses for both normal and smartcache paths. Added parser tests for JSON-LD stock, Brico Dépôt smartcache fragments, embedded Vue stock, Costway presale/cooler/split filtering, and Maison Energy quote-only/preorder precedence. Backend tests: 109.
...
- Live local French dry-run on 2026-07-06 (second France batch after deferring Brico Dépôt France): all 13 active French adapters completed; snapshot preview contained 44 available/presale products and the alert filter selected 9. Costway France returned 4 preorder products with `presale=true`, correctly excluded from immediate-stock alerts.
