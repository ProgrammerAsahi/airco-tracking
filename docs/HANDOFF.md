...
- Azure resource group: `airco-tracker-rg` (9 resources moved here from `airco-tracker-nl-rg`; UAMIs and EmailService remain in `airco-tracker-nl-rg` as they do not support move)
...
- Azure resource move 2026-07-05: created `airco-tracker-rg` and moved 9 resources (Storage, ACR, KeyVault, LogAnalytics, ACS, ManagedEnvironment, ContainerApp, Job) from `airco-tracker-nl-rg`. UAMIs (`aircontrack-identity`, `airco-github-deployer`) and Communication EmailService remain in `airco-tracker-nl-rg` (not movable). Recreated 6 role assignments on the new RG's resources (5 runtime + 1 deployer). Manual job execution `airco-tracker-job-0s5hooj`: Succeeded. Frontend API verified 2026-07-05T16:39Z: 28 sites, 19 available products, 0 stale sites. GitHub Actions `AZURE_RESOURCE_GROUP` variable updated to `airco-tracker-rg` on both repos.
