...
- `alertoutboxpending`：单一 hot pending partition 中的权威 enqueue journal。每个 v2 row 都包含完整、不可变的事件和受 ETag 保护、可恢复的 publisher lease；publisher 会先修复 archive 再确认 journal。只含 pointer 的旧 row 在 archive 仍可能并发写入时不会被删除。
...
- Web、scanner、retention、outbox publisher、fan-out 和 email delivery 使用按职责分离的身份。新权限在 Azure RBAC 支持的范围内限制到具体 Blob container、Service Bus entity 或 Table。
...
- Publisher 通过单一 `alertoutboxpending` partition 读取完整事件，无需扫描分片 archive table；旧数据迁移会耗尽全部 continuation pages 后才记录完成。Retention 默认不限行数，使用 240 秒 runtime budget，仍有 backlog 时会输出 warning。
...
Bicep 还会注入 `AZURE_STORAGE_ACCOUNT_URL`、`AZURE_CLIENT_ID`、`ACS_ENDPOINT`、`EMAIL_FROM` 和正常 scanner 配置。`EMAIL_TO` 只属于本地/direct mode，不是生产 subscriber 数据源。本地或单 replica worker 可以保留 `EMAIL_RATE_LIMIT_BACKEND=local`；任何多 replica 部署都必须使用 `azure_table`。
...
# 两个 smoke tests 均通过后才执行：
./scripts/migrate-runtime-identities.sh --apply
```

Foundation-only 模式只创建 resources、identities、RBAC 和更新后的 least-privilege GitHub role，不移动生产 workload。Web deploy 随后会创建 cleanup job，并在切流前立即启动验证；后端 application deploy 会验证 candidate 和按依赖排序的 jobs。Identity migration 第一次只做只读审计，绝不会自动删除旧 grants；只有两个 smoke tests 都通过后，Owner 才能使用 `--apply`。对于已经完整的环境，完整 `deploy-azure.sh` 仍然可用；greenfield bootstrap 若尚无 Web cleanup job，会明确延后 migration audit 而不是失败。详见[运行时加固与迁移](./HARDENING.zh.md)。新建 RBAC 可能需要数分钟传播；若首次 application deployment 因此失败，请等待后重试。
