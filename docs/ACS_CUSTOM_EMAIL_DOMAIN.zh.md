...
2. 对应 delivery-ledger rows 达到 `sent`，email-worker log 显示 ACS accepted，并且没有 ACS `429`。
3. 两个 inbox 都实际收到邮件，同时检查 spam/junk。
4. 查看原始邮件头，确认用户可见 From 是 `airco-tracker.eu`、SPF 通过、DKIM 通过且签名域对齐；发布 DMARC 后确认 DMARC 通过，同时确认 Return-Path/MailFrom 与自定义域对齐。
5. Stock subscription 和两条 queues 的 active、scheduled、transfer-dead-letter 和 dead-letter counts 全部回到零。
...
- 配置真实、有人监控的 mailbox 或转发地址，例如 `support@airco-tracker.eu`，并用作 Reply-To。应用当前尚未添加 Reply-To，所以这是待实现功能，不是只改 DNS 就能完成。
- 通过选择的收信服务商配置真实 MX。ACS 只负责 outbound，不提供 inbox。Microsoft 建议配置 MX，因为没有收信路径的 sender domain 可能被判定为较低 reputation。
- 启用 DMARC report 前，准备有人监控的 aggregate-report mailbox，例如 `dmarc@airco-tracker.eu`。
- 先以 monitoring mode 发布 DMARC：
...
- 检查 reports 和 authentication alignment 后，再逐步改为 `p=quarantine`，最后改为 `p=reject`。盘点所有合法 sender 之前不得直接发布严格 reject policy。
...
当前流水线已有 durable accepted-send tracking、subscription cancellation，以及 ACS 平台临时的 managed hard-bounce suppression，但还没有全部 high-volume controls：

- 没有消费 Event Grid `Microsoft.Communication.EmailDeliveryReportReceived`，不会记录最终 `Delivered`、`Bounced`、`Suppressed`、`Quarantined`、`FilteredSpam` 或 `Failed`；
- 不会在应用层自动 suppress 持续 hard bounce 的 recipient；
- 没有独立于付费权益到期的即时 email-only opt-out；
- 没有可用的 unsubscribe link 或标准 one-click unsubscribe headers；
- 没有 application Reply-To；
- 没有 ACS diagnostic setting，也没有 final delivery-failure spike / ACS `429` alerts。

这些能力完成之前，不能在 quota questionnaire 中声称已经具备；应先实现，或如实说明当前的人工/平台控制。ACS accepted 不是最终 inbox-delivery 结果。
...
确认 deployment 和 targeted canary 后才算结束 incident。排查期间保持 custom domain linked；未被应用选中的 linked domain 不会强迫应用使用它。只有确认应用已经使用 `AzureManagedDomain` 后才考虑断开：
