...
`deploy-application.sh` 会先构建 immutable candidate。如果已存在旧版本，它会通过 `az containerapp job start --image <candidate>` 用候选镜像单独运行一次 recipient reconciler；该执行沿用已部署 identity 和依赖，但不会改动任何生产 job definition。Canary 成功后才进行 Bicep 更新，随后对新镜像依次执行 reconciler → scanner → publisher 验证。如果部署或任何部署后检查失败，`EXIT` guard 会自动用已记录的 previous image 重部署所有 application workloads，并再次验证 reconciler。
