...
本文记录 2026 年 7 月加固版本新增的库存 freshness 契约、出站 URL 边界、有界抓取、状态/提醒保留、pending outbox 索引，以及只能由 Owner 执行的运行身份迁移。该版本和迁移已经完成生产验证；下述流程继续作为权威回滚与新环境 runbook 保留。
...
- `${prefix}-alert-publisher`：仅供 publisher；读取/更新 pending 与 archive outbox rows，并且只能向 `stock-events` topic 发送。
- `${prefix}-alert-fanout`：仅供 reconciler/coordinator/fan-out；读取最小 recipient projection、消费 `email-fanout`，并向两条 fan-out queues 发送只含 opaque recipient 的 jobs。
- `${prefix}-alert-email`：仅供邮件 worker；保留原有 queue/table/ACS 权限，并且只读取 `unsubscribe-signing-key`。
- `${prefix}-alert-delivery-report`：仅供 ACS delivery-report worker 与 DLQ cleanup；消费 delivery-event queue 并更新 delivery/index/suppression state，不拥有 stock publish 或 email send 权限。
...
- Backend image/commit `1dd3017607ea0f2e6c72a223cd5aa0b6b5f5d24e` 通过 413/413 unit tests 和 CI workflow `29963989458`；deploy workflow `29963989419` 通过必需的 production 审批后完成。兼容的 frontend 为 commit `d097c75c9850946be024920677eba6761174ac48`、revision `airco-tracking-web--0000066`。
- Owner 只在 workload bindings 和替代 grants 全部验证后执行了 `migrate-runtime-identities.sh --apply`。部署后的 dry-run 是干净 no-op：所有精确替代项仍存在、枚举的旧宽泛 grant 均已移除，Bicep 重部署也没有重新创建旧 grant。
- Scanner、后端 retention、Web 认证 cleanup、publisher、reconciler/coordinator/fan-out、email 与 delivery-report workloads 均使用上文精确身份。Health/readiness、本地化法律内容、匿名库存拒绝、Service Bus queue/subscription health 和生产 jobs execution 均通过。
- 第一次生产 retention run 暴露了本地 mocks 隐藏的真实 SDK 契约：`TableClient.query_entities` 需要位置参数 `query_filter`。Commit `1dd3017` 显式传入空 filter 并增加回归断言；重新部署后真实生产 retention execution 成功。
- 真实 ACS 验证邮件已投递到彼此独立的 Outlook 和 Gmail provider。Custom-domain sender、support `Reply-To`、SPF、DKIM 和 DMARC 均验证通过。发布窗口内不存在 active entitled alert recipient，因此完整 fan-out canary 安全地没有生成 delivery job；没有绕过权益检查。
- 剩余外部门禁为尚在处理的 ACS 配额提升和 sender warm-up、私有法律/商业事实补齐与专业审查，以及 Stripe live activation。在这些条件满足前，Stripe 保持 test mode 并 fail closed。
