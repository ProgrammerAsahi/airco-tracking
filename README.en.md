...
French adapters also split immediate stock from presale inventory: `Pré-commande`, `Expédition à partir`, `livraison prévue semaine`, `Sur commande`, and multi-week lead times are shown as presale and do not trigger immediate-stock email alerts. Electro Dépôt France reads the embedded Vue JSON `stock` value; EcoFlow France reads its first-party Shopify collection and product pages, trusts the variant `available` flag over marketing preorder copy, and uses the official 5,100/6,100 BTU WAVE 2/3 capacities.

E.Leclerc France does not scrape its anti-bot storefront pages. It uses the storefront's first-party same-origin frontend API instead: product search discovers candidates at low frequency, approximately every 12 hours, while bulk product details refresh known-SKU stock every ten minutes. A product must match `family.code=climatiseur` exactly; accessories, air coolers, and fans are rejected. The offer `additionalFields` value `availability-status` is authoritative. Immediate stock requires `in-stock`, numeric stock greater than zero, an active date window, a valid positive EUR price, and a nonempty seller. Presale accepts only `preorder`, `unlimited-preorder`, `forthcoming`, or `future-stock`, with the same active-window, price, and seller requirements, while stock may be zero. `shipped-under`, `temporarily-unavailable`, `unavailable`, missing or unknown statuses, and `in-stock` with stock at or below zero are unavailable. With multiple offers, the adapter selects the cheapest immediate offer, otherwise the cheapest presale offer, otherwise unavailable. Stock truth comes from this first-party live API; outbound user links use the approved Awin deep link. The current E.Leclerc Awin Product Feed excludes air conditioners, so runtime no longer depends on an Awin Product Feed data key.

Trotec France reads the Algolia product index used by its official storefront on every scan. It is the sole authority for live stock and presale classification; `sold_out=Oui` remains a hard veto even when `availability_status` says `En stock`. Approved Awin advertiser `62319` is used only through Link Builder: validated `fr.trotec.com/shop/...` product URLs are submitted to the official API in one batch of at most 100 and successful results are cached for one day. An `affiliate_url` is attached only after validating the returned HTTPS Awin host, advertiser, publisher, and final Trotec destination. A timeout, per-item rejection, or missing token falls back to the canonical Trotec URL and can never fail or stale the stock scan. `Product.url` remains the stable inventory, deduplication, and transition-event identity; the web UI and alert email disclose affiliate links before a click.

AliExpress has a shared Affiliate Open Platform client plus country-bound FR and NL inspection adapters. Every search and SKU-detail request includes the actual delivery destination, so a product returned for France is never reused as proof of delivery to the Netherlands (or vice versa). The approved SKU contract exposes item, price, tax, shipping-origin, delivery-window, and SKU-property data, but no documented inventory or orderability field. Therefore AliExpress is currently diagnostic-only and is deliberately absent from the production adapter registry: a returned SKU, price, delivery estimate, or promotional link is never treated as stock. See [docs/ALIEXPRESS_INTEGRATION.md](docs/ALIEXPRESS_INTEGRATION.md).

Maison Energy lets `Non disponible`/`Demande de devis` override schema `PreOrder` so quote-only products do not alert as stock. H2R Équipements reads only the camper/van `climatisation nomade` category, where only `En stock` is immediate and `Sur commande`/`Retour en stock prévu` never alerts as stock; Obelink France reads public JSON-LD category and product data for mobile/split camping air conditioners; Narbonne Accessoires trusts the visible `Livraison à Domicile` block rather than schema stock so store-only pickup is not reported as delivery stock; Mon Camping Car displays `BackOrder`/`Disponible à partir` products as presale. Costway France has been removed from the active registry and retained as a deferred adapter because production requests consistently return HTTP 403; Boulanger, Brico Dépôt France, Cdiscount, and the other direct-403 French retailers are also disabled so timeouts, anti-bot pages, or JS shells are not treated as stock sources. Action France currently returns mostly coolers/fans for the query and is strictly filtered.
...
Key Vault holds only required third-party adapter credentials. The container can load the publisher token for the Trotec France Awin Link Builder through Managed Identity:
...
Only the Awin Link Builder Batch API is called, and its bearer token appears only in the `Authorization` header. Legacy Create-a-Feed URLs that embed a data-feed key are intentionally unsupported. First-party stock classification completes before link generation; an API failure falls back to the canonical URL. No CMP is deployed yet, so every API-returned link is explicitly overridden to `cons=0`: Awin sets no cookie and transfers no click identifier to the merchant, so the current path cannot be relied upon for commission attribution. Both web and email disclose the affiliate relationship before a click; `cons=1` may be sent only after real per-journey consent management exists.
